...
class StatusBarCompatLollipop {
  ...
  /**
   * set StatusBarColor <p> 1. set Flags to call setStatusBarColor 2. call setSystemUiVisibility to
  ...
  /**
   * translucentStatusBar(full-screen) <p> 1. set Flags to full-screen 2. set FitsSystemWindows to
   * false
   *
  ...
  /**
   * compat for CollapsingToolbarLayout <p> 1. change to full-screen mode(like
   * translucentStatusBar). 2. cancel CollapsingToolbarLayout's WindowInsets, let it layout as
   * normal(now setStatusBarScrimColor is useless). 3. set View's FitsSystemWindow to false. 4. add
   * Toolbar's height, let it layout from top, then add paddingTop to layout normal. 5. change
   * statusBarColor by AppBarLayout's offset. 6. add Listener to change statusBarColor
   */
