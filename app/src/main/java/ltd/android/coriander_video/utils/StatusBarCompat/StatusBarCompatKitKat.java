package ltd.android.coriander_video.utils.StatusBarCompat;
...
/**
 * After kitkat add fake status bar Created by qiu on 8/27/16.
 */
...
class StatusBarCompatKitKat {
  ...
  /**
   * set StatusBarColor <p> 1. set Window Flag : WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS
   * 2. removeFakeStatusBarViewIfExist 3. addFakeStatusBarView 4. addMarginTopToContentChild 5.
   * cancel ContentChild's fitsSystemWindow
   */
  ...
  /**
   * translucentStatusBar <p> 1. set Window Flag : WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS
   * 2. removeFakeStatusBarViewIfExist 3. removeMarginTopOfContentChild 4. cancel ContentChild's
   * fitsSystemWindow
   */
  ...
  /**
   * compat for CollapsingToolbarLayout <p> 1. set Window Flag : WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS
   * 2. set FitsSystemWindows for views. 3. add Toolbar's height, let it layout from top, then add
   * paddingTop to layout normal. 4. removeFakeStatusBarViewIfExist 5. removeMarginTopOfContentChild
  ...
  static void setStatusBarColorForCollapsingToolbar(Activity activity,
    ...
    CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) appBarLayout
        ...
        if (Math.abs(verticalOffset) > appBarLayout.getHeight() - collapsingToolbarLayout
          ...
          if (statusView.getAlpha() == 0) {
            ...
            statusView.animate().alpha(1f)
                .setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
          }
        } else {
