...
class SearchViewModel : BaseViewModel() {
     ...
     */
    fun getMovie(page: Int, keyword: String, isRefresh: Boolean, start: () -> Unit, finally: () -> Unit) {
        launchOnUITryCatch(
            {
                val map = mapOf(
                    "pageSize" to AppConfigure.globalPageSize,
                    "page" to page,
                    "keyword" to keyword
                )
                start()
                val baseResponse = withContext(IO) { MovieAPi.instance.getSearchAsync(map) }.await()
